...
```bash
discounts$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
discounts$ sam local start-api
discounts$ curl http://localhost:3000/
```
...
```bash
discounts$ sam logs -n HelloWorldFunction --stack-name discounts --tail
```
...
```bash
sam delete --stack-name discounts
```
